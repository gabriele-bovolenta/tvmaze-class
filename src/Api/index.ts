...
export const getShowsBySearch = async (query: string) => {
  ...
  const mappedData: ShowType[] = data.map((el) => ({
    id: el.show.id,
    title: el.show.name,
    image: el.show.image?.medium === undefined || el.show.image?.medium === ''? require('../Asset/images/background.jpeg') : el.show.image?.medium,
...
export const getShowsById = async (id: number) => {
  ...
  const mappedData: ShowDetailType = {
    ...
    image: data.image?.original === undefined || data.image?.original === '' ? require('../Asset/images/background.jpeg') : data.image?.original,
